...
function levenshtein(a, b) {
  ...
  for (let i = 0; i < m + 1; i += 1) dp[i][0] = i;
  for (let j = 0; j < n + 1; j += 1) dp[0][j] = j;
...
function drawPoiMarkers() {
  ...
  visible.forEach((poi) => {
    ...
    const marker = L.marker([poi.lat, poi.lng], { icon }).bindPopup(
      `<b>${normalizeUiText(poi.name)}</b><br><small>${normalizeUiText(categoryLabel)} • ${normalizeUiText(poi.city)}</small><p style="margin:6px 0 8px">${normalizeUiText(poi.description)}</p><a href="${poi.maps}" target="_blank" rel="noreferrer">Google Maps</a>`
    );
